...
都是通过 deps 的变化 来更新回调函数；

### useMemo 和 useCallback

初始渲染执行顺序依次为 useMemo -> useCallback -> useEffect
useCallback(fn, deps) 其实相当于 useMemo(() => fn, deps)，所以说：使用 useMemo 完全可以实现 useCallback。

### 不同点

useEffect 就不多说了，相当于函数式编程通往命令式的一个通道-->即 class 对应的生命周期概念。
